<template>
  <div class="meedu-main-body">
    <back-bar class="mb-30" title="编辑话题分类"></back-bar>
...
export default {
  ...
  methods: {
    detail() {
      this.$api.Course.Topic.Category.Detail(this.user.id).then((res) => {
        var data = res.data;
    ...
    confirm() {
      ...
      this.loading = true;
      this.$api.Course.Topic.Category.Update(this.user.id, this.user)
        .then(() => {
