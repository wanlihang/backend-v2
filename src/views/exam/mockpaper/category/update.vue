<template>
  <div class="meedu-main-body">
    <back-bar class="mb-30" title="编辑模拟试卷分类"></back-bar>
...
export default {
  ...
  methods: {
    params() {
      this.$api.Exam.Mockpaper.Category.Create().then((res) => {
        this.categories = res.data.categories;
      });
    },
    detail() {
      this.$api.Exam.Mockpaper.Category.Detail(this.user.id).then((res) => {
    ...
    confirm() {
      ...
      this.loading = true;
      this.$api.Exam.Mockpaper.Category.Update(this.user.id, this.user)
        .then(() => {
