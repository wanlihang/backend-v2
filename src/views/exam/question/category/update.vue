<template>
  <div class="meedu-main-body">
    <back-bar class="mb-30" title="编辑试题分类"></back-bar>
...
export default {
  ...
  methods: {
    params() {
      this.$api.Exam.Question.Category.Create().then((res) => {
        this.categories = res.data.categories;
      });
    },
    detail() {
      this.$api.Exam.Question.Category.Detail(this.user.id).then((res) => {
    ...
    confirm() {
      ...
      this.loading = true;
      this.$api.Exam.Question.Category.Update(this.user.id, this.user)
        .then(() => {
