<template>
  <div class="float-left" v-if="init">
    ...
    <div class="float-left mb-15">
      ...
      <div class="float-left">
        <el-select class="w-400px" multiple v-model="selectedAnswers">
          <el-option
...
export default {
  ...
  watch: {
    ...
    selectedAnswers(newVal) {
      this.form.answer = newVal.join(",");
    },
  },
  mounted() {
    if (this.question) {
      ...
      // 答案
      let answers = [];
      if (this.question.answer && typeof this.question.answer === "string") {
        answers = this.question.answer.split(",");
      }
