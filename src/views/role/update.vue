<template>
  <div class="meedu-main-body">
    <back-bar class="mb-30" title="编辑VIP角色"></back-bar>
...
export default {
  ...
  methods: {
    detail() {
      this.$api.Role.Detail(this.course.id).then((res) => {
        var data = res.data;
        this.course.id = data.id;
        this.course.is_show = data.is_show;
        this.course.name = data.name;
        this.course.expire_days = data.expire_days;
    ...
    confirm() {
      ...
      this.loading = true;
      this.$api.Role.Update(this.course.id, this.course)
        .then(() => {
