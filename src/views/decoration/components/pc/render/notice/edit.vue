<template>
  <div class="meedu-dialog-mask">
    <div class="meedu-dialog-box">
      <div class="meedu-dialog-header">编辑公告</div>
      <div class="meedu-dialog-body" v-if="form">
...
export default {
  ...
  methods: {
    getData() {
      this.$api.System.Announcement.Detail(this.id).then((res) => {
        this.form = res.data;
      });
    },
    formValidate() {
    ...
    confirm() {
      ...
      this.loading = true;
      this.$api.System.Announcement.Update(this.id, this.form)
        .then(() => {
